...
- closure-check、Skill、plugin、decision register、milestone 与 Beta evidence
  validator 均通过。
- 确定性 120 文件源码包已 build/verify；最终 SHA-256 记录于
  `dist/SuperLuna-0.2.0-alpha.96.zip.sha256.txt`。
- 上述结果只证明本地 Alpha 96 候选，不代表真实 App 闭环或公开 Beta
...
- Controller 152 / Skill revision `2026-08-19.109` 修复 Alpha95 实机仍从旁路返回笼统
  “退休记录缺失”的问题。所有缺失退休记录的入口现在统一经过只读
  `retirement_recovery_diagnostic`：逐项返回版本、task/reviewer、rollover、repository、零副作用、
  slot、限流事实、browser binding 与上一代 authorization 的布尔结果及完整稳定缺项列表。
  guard 在 orphan provisioning 之前执行该诊断；直接 rollover 命令不再抛出泛化 controller error；
  `diagnose-rate-limit-retirement` 可在零 state/registry 写入、零 browser/Chat 访问下单独核对入口版本漂移。
...
- Controller 149 / Skill revision `2026-08-19.106` 修复 Alpha92 实机中 replacement Chat 已持久绑定、
  ...
  repository identity、上一 generation、授权→绑定→替换时间链、canonical provisioned browser/provider
  receipt、当前代零 request/response，以及账户门由同任务限流后全局零 slot。成功只生成一个确定性 rebuild
  identity 与一个退休记录；缺少 binding receipt、存在任一 slot 或身份漂移均保持 registry 不变并失败关闭。
...
- Controller 147 / Skill revision `2026-08-19.104` 修复 Alpha90 实机仍返回普通 guard 通过的问题。
  ...
  恢复未发送 submission，并将创建/绑定唯一 RDATE 作为同回合 barrier；绑定后仅复用同一 automation。
...
- Controller 145 / Skill revision `2026-08-19.102` 修复 replacement startup 真实限流被顶层
  ...
  复用一个 RDATE 单次恢复，并明确显示该恢复是否已绑定；技术阻断始终 `user_choice_required=false`。
- Controller 144 / Skill revision `2026-08-19.101` 为“旧 provisioning 已消费但零副作用”增加一次
  原子 reconcile。只有同一 task/state/reviewer generation/repository identity 严格匹配，且没有 browser
  init、Chat identity、submission/read receipt 或活动/过期不确定 slot 时，guard 才自动返回唯一恢复
  命令；恢复后仍只取得同代唯一 startup slot。证据缺失、身份漂移、已有副作用或并发均失败关闭。
...
- 当前源码仓库回归 361/361、控制器回归 250/250；已通过新增的 Chat 八轮换卷、限流 Chat
