...
如需只读观察单个运行，可执行 `lcrl observe-run --state <state-file>`。它只读取现有
状态，返回上述五种用户状态、当前阶段、最近一次带证据的实质进展时间、距今分钟数，
以及默认 20 分钟阈值下的 `possibly_stuck`。`等待 Chat` 永远不会被判为卡住；没有
实质进度事件时不臆测时间，也不发出卡住告警。可用 `--threshold-minutes` 调整阈值，
用 `--at` 固定观察时刻以便复核。
