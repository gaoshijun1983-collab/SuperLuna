...
当前公开开源候选版本是 `0.2.0-alpha.53`。它仍是供技术测试者使用的早期 Alpha，尚不是
Public Beta。当前源码为控制器 109 / Skill revision `2026-08-13.66`：仓库复测工作区探针兼容
Windows，用户可见的单次等待说明保持
简短中英文，并修复等待任务读到完整回复后仍被允许结束的问题；现在必须由同一任务在同一回合恢复、
应用结果并准备下一轮提交。同时 SuperLuna 自身开发与真实
