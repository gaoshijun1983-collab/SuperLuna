...
当前源码候选是 `0.2.0-alpha.46`，最新归档仍为 Alpha 27。控制器 54 / Skill revision
`2026-08-12.8` 增加多 state 只读总览，在每条任务上返回五种用户状态、阶段、最近实质证据和精确 20 分钟卡住判定，
并汇总状态计数；控制器 53 将卡住边界覆盖“正在开发”和“正在按 Chat 意见修改”，并明确上下文压缩后恢复门禁必须继续携带同一个稳定任务 identity。
控制器 52 增加不改状态字节的单任务只读观察器；同时增加
