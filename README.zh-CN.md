...
当前源码候选版本是 `0.2.0-alpha.63`。它仍是供技术测试者使用的早期 Alpha，尚不是
Public Beta。当前源码为控制器 119 / Skill revision `2026-08-14.76`：任何时候只保留一个活动
评审 Chat；每个 Chat 最多完成 8 次正式评审，第 9 次之前自动换卷。只要某个 Chat 真实出现一次
...
- 一项实施任务、同一时刻一个活动 reviewer Chat；每卷最多 8 次正式评审。标题和焦点不是身份。
