...
`2026-08-12.5` 增加只读 `lcrl observe-run` 入口：它从现有 state 推导五种用户状态、当前阶段、
有证据的进度年龄和有限的可能卡住信号，不写 state、不读 Chat、也不取得 lease。`2026-08-12.4`
把发送前门改成持久化的一次性授权事实：仅持有重开 lease 并传入已知 state
